...
export default function Header({ loggedIn, pathname, onLogo, onMovies, onSavedMovies, onRegister, onSignin, onProfile }) {
  return (
      <header className={ `header ${ pathname === '/' ? "header_blue" : "header_black" }` }>
        <div className="header__container">
          <ButtonLogo onClick={ onLogo }/>
          { loggedIn ? <Navigation pathname={pathname} loggedin={loggedIn} onMovies={onMovies} onSavedMovies={onSavedMovies} onProfile={onProfile}/> : null}
